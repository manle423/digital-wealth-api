...
export class RecommendationService {
  ...
  async generateRecommendations(userId: string): Promise<Recommendation[]> {
    try {
      this.logger.info(
        '[generateRecommendations] Starting recommendation generation',
        { userId },
      );
      ...
      const recommendations = await Promise.all(
        filteredRecommendations.map((rule) =>
          this.createRecommendation(userId, rule),
        ),
      );
      ...
      this.logger.error(
        '[generateRecommendations] Error generating recommendations',
        error,
      );
      throw error;
    }
  ...
  async getActiveRecommendations(userId: string): Promise<Recommendation[]> {
    try {
      ...
      this.logger.error(
        '[getActiveRecommendations] Error getting active recommendations',
        error,
      );
      throw error;
    }
  ...
  async getRecommendationsByType(
    ...
    try {
      ...
      return await this.recommendationRepository.findByUserIdAndType(
        userId,
        type,
      );
    } catch (error) {
      this.logger.error(
        '[getRecommendationsByType] Error getting recommendations by type',
        error,
      );
      throw error;
    }
  ...
  async markAsViewed(userId: string, recommendationId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        '[markAsViewed] Error marking recommendation as viewed',
        error,
      );
      throw error;
    }
  ...
  async markAsDismissed(
    ...
    try {
      ...
      this.logger.error(
        '[markAsDismissed] Error marking recommendation as dismissed',
        error,
      );
      throw error;
    }
  ...
  async markAsCompleted(
    ...
    try {
      ...
      this.logger.error(
        '[markAsCompleted] Error marking recommendation as completed',
        error,
      );
      throw error;
    }
  ...
  async submitFeedback(
    ...
    try {
      this.logger.info('[submitFeedback]', {
        userId,
        recommendationId,
        rating,
      });

      await this.recommendationRepository.updateUserFeedback(
        recommendationId,
        feedback,
        rating,
      );
  ...
  async getRecommendationStats(userId: string): Promise<any> {
    try {
      ...
      this.logger.error(
        '[getRecommendationStats] Error getting recommendation stats',
        error,
      );
      throw error;
    }
  ...
  async getRecommendationsByStatus(
    ...
    try {
      ...
      return await this.recommendationRepository.findByUserIdAndStatus(
        userId,
        status,
      );
    } catch (error) {
      this.logger.error(
        '[getRecommendationsByStatus] Error getting recommendations by status',
        error,
      );
      throw error;
    }
  ...
  private async buildFinancialProfile(
    ...
    const [
      ...
      this.financialAnalysisService.getLatestMetric(
        userId,
        MetricType.LIQUIDITY_RATIO,
      ),
      this.financialAnalysisService.getLatestMetric(
        userId,
        MetricType.DEBT_TO_ASSET_RATIO,
      ),
      this.financialAnalysisService.getLatestMetric(
        userId,
        MetricType.INVESTMENT_RATIO,
      ),
      this.financialAnalysisService.getLatestMetric(
        userId,
        MetricType.DIVERSIFICATION_INDEX,
      ),
    ]);
  ...
  private getRecommendationRules(): RecommendationRule[] {
    return [
      // CRITICAL PRIORITY RULES
      {
        ...
        description:
          'Tỷ lệ thanh khoản của bạn quá thấp (< 5%). Điều này có thể gây rủi ro tài chính nghiêm trọng khi có sự cố bất ngờ.',
        rationale:
          'Quỹ khẩn cấp là nền tảng của an toàn tài chính. Chuyên gia khuyến nghị nên có ít nhất 3-6 tháng chi tiêu trong tài khoản thanh khoản.',
        actionSteps: [
          {
            step: 1,
          ...
          {
            step: 3,
            description: 'Thiết lập chuyển khoản tự động',
            isCompleted: false,
          },
          {
            step: 4,
            description: 'Đạt mục tiêu 3 tháng chi tiêu trong 6 tháng',
            isCompleted: false,
          },
        ],
        expectedImpact: {
...

      {
        ...
        description:
          'Tỷ lệ nợ/tài sản vượt ngưỡng an toàn (70%). Cần hành động ngay để tránh rủi ro tài chính.',
        rationale:
          'Tỷ lệ nợ cao có thể dẫn đến khó khăn thanh toán, ảnh hưởng đến điểm tín dụng và hạn chế khả năng đầu tư.',
        actionSteps: [
          {
            step: 1,
            description: 'Liệt kê tất cả các khoản nợ và lãi suất',
            isCompleted: false,
          },
          {
            step: 2,
          ...
          {
            step: 3,
            description: 'Cắt giảm chi tiêu không cần thiết',
            isCompleted: false,
          },
          {
            step: 4,
            description: 'Xem xét tái cấu trúc nợ với lãi suất thấp hơn',
            isCompleted: false,
          },
        ],
        expectedImpact: {
      ...
      {
        condition: (profile) =>
          profile.liquidityRatio >= 5 && profile.liquidityRatio < 15,
        priority: RecommendationPriority.HIGH,
        type: RecommendationType.INCREASE_SAVINGS,
        title: 'Tăng cường quỹ thanh khoản',
        description:
          'Tỷ lệ thanh khoản của bạn ở mức khá, nhưng nên tăng lên 20-25% để đảm bảo an toàn tài chính tốt hơn.',
        rationale:
          'Quỹ thanh khoản đầy đủ giúp bạn đối phó với các tình huống bất ngờ mà không cần vay nợ.',
        actionSteps: [
          {
            step: 1,
            description: 'Tăng tỷ lệ tiết kiệm lên 15% thu nhập',
            isCompleted: false,
          },
          {
            step: 2,
            description:
              'Đầu tư vào các sản phẩm thanh khoản cao như tiền gửi có kỳ hạn ngắn',
            isCompleted: false,
          },
          {
            step: 3,
            description: 'Xem xét mở thêm tài khoản tiết kiệm với lãi suất cao',
            isCompleted: false,
          },
        ],
        expectedImpact: {
      ...
      {
        condition: (profile) =>
          profile.investmentRatio < 20 && profile.liquidityRatio > 20,
        priority: RecommendationPriority.HIGH,
        type: RecommendationType.INVESTMENT_OPPORTUNITY,
        title: 'Bắt đầu đầu tư để tăng trưởng tài sản',
        description:
          'Bạn có quỹ thanh khoản tốt nhưng tỷ lệ đầu tư còn thấp. Đây là cơ hội để tăng trưởng tài sản dài hạn.',
        rationale:
          'Đầu tư là cách hiệu quả để chống lạm phát và tăng trưởng tài sản theo thời gian.',
        actionSteps: [
          {
            step: 1,
            description: 'Học về các loại hình đầu tư cơ bản',
            isCompleted: false,
          },
          {
            step: 2,
            description: 'Bắt đầu với quỹ đầu tư cân bằng hoặc ETF',
            isCompleted: false,
          },
          {
            step: 3,
            description: 'Đầu tư định kỳ 10-15% thu nhập hàng tháng',
            isCompleted: false,
          },
          {
            step: 4,
            description: 'Đa dạng hóa danh mục đầu tư',
            isCompleted: false,
          },
        ],
        expectedImpact: {
      ...
      {
        condition: (profile) =>
          profile.diversificationIndex < 50 && profile.investmentRatio > 20,
        priority: RecommendationPriority.HIGH,
        type: RecommendationType.DIVERSIFICATION,
        title: 'Đa dạng hóa danh mục đầu tư',
        description:
          'Chỉ số đa dạng hóa của bạn thấp. Danh mục đầu tư cần được phân tán rủi ro tốt hơn.',
        rationale:
          'Đa dạng hóa giúp giảm rủi ro đầu tư bằng cách phân tán vốn vào nhiều loại tài sản khác nhau.',
        actionSteps: [
          {
            step: 1,
            description: 'Phân tích danh mục đầu tư hiện tại',
            isCompleted: false,
          },
          {
            step: 2,
            description: 'Đầu tư vào các lĩnh vực/khu vực địa lý khác nhau',
            isCompleted: false,
          },
          {
            step: 3,
            description: 'Cân bằng giữa cổ phiếu, trái phiếu và bất động sản',
            isCompleted: false,
          },
          {
            step: 4,
            description: 'Đánh giá và cân bằng lại danh mục định kỳ',
            isCompleted: false,
          },
        ],
        expectedImpact: {
      ...
      {
        condition: (profile) =>
          profile.debtToAssetRatio > 30 && profile.debtToAssetRatio <= 50,
        priority: RecommendationPriority.MEDIUM,
        type: RecommendationType.DEBT_CONSOLIDATION,
        title: 'Xem xét hợp nhất nợ để tối ưu lãi suất',
        description:
          'Với tỷ lệ nợ hiện tại, bạn có thể tiết kiệm chi phí bằng cách hợp nhất các khoản nợ.',
        rationale:
          'Hợp nhất nợ có thể giúp giảm lãi suất tổng thể và đơn giản hóa việc quản lý tài chính.',
        actionSteps: [
          {
            step: 1,
            description: 'So sánh lãi suất các khoản nợ hiện tại',
            isCompleted: false,
          },
          {
            step: 2,
            description: 'Tìm hiểu các gói vay hợp nhất nợ',
            isCompleted: false,
          },
          {
            step: 3,
            description: 'Tính toán lợi ích tài chính từ việc hợp nhất',
            isCompleted: false,
          },
          {
            step: 4,
            description: 'Thực hiện hợp nhất nếu có lợi',
            isCompleted: false,
          },
        ],
        expectedImpact: {
...

      {
        ...
        description:
          'Tình hình tài chính của bạn ổn định. Đây là thời điểm tốt để đặt và theo đuổi các mục tiêu tài chính dài hạn.',
        rationale:
          'Mục tiêu tài chính rõ ràng giúp định hướng các quyết định đầu tư và tiết kiệm hiệu quả hơn.',
        actionSteps: [
          {
            step: 1,
            description: 'Xác định các mục tiêu tài chính 5-10 năm tới',
            isCompleted: false,
          },
          {
            step: 2,
            description: 'Tính toán số tiền cần thiết cho từng mục tiêu',
            isCompleted: false,
          },
          {
            step: 3,
            description: 'Lập kế hoạch tiết kiệm và đầu tư cụ thể',
            isCompleted: false,
          },
          {
            step: 4,
            description: 'Theo dõi tiến độ hàng quý',
            isCompleted: false,
          },
        ],
        expectedImpact: {
          ...
          riskLevel: 'LOW',
          description:
            'Đạt được các mục tiêu tài chính quan trọng như mua nhà, nghỉ hưu',
        },
        expiresInDays: 90,
      },
      ...
      // LOW PRIORITY RULES
      {
        ...
        description:
          'Tình hình tài chính của bạn rất tốt. Hãy xem xét các chiến lược tối ưu thuế và đầu tư nâng cao.',
        rationale:
          'Với nền tảng tài chính vững chắc, việc tối ưu thuế có thể mang lại lợi ích đáng kể.',
        actionSteps: [
          {
            step: 1,
            description: 'Tìm hiểu các ưu đãi thuế cho đầu tư dài hạn',
            isCompleted: false,
          },
          {
            step: 2,
            description: 'Xem xét các sản phẩm bảo hiểm nhân thọ có tích lũy',
            isCompleted: false,
          },
          {
            step: 3,
            description: 'Lập kế hoạch nghỉ hưu chi tiết',
            isCompleted: false,
          },
        ],
        expectedImpact: {
  ...
  private async filterAndPrioritize(
    ...
    for (const rule of potentialRecommendations) {
      const similarRecommendations =
        await this.recommendationRepository.findSimilarRecommendations(
          userId,
          rule.type,
          30,
        );
  ...
  private async clearRecommendationCaches(userId: string): Promise<void> {
    try {
      const keysToDelete = [`${RedisKeyPrefix.RECOMMENDATIONS}:${userId}`];

      await Promise.all(
        keysToDelete.map((key) => this.redisService.delWithPrefix(key)),
      );

      this.logger.debug(
        '[clearRecommendationCaches] Cleared recommendation caches',
        { userId },
      );
    } catch (error) {
      this.logger.error(
        `[clearRecommendationCaches] Error clearing caches: ${error.message}`,
        { userId },
      );
    }
  }
