...
export class RiskProfileService {
  ...
  async getAllRiskProfiles(
    query?: GetRiskProfilesDto,
  ): Promise<{ data: RiskProfile[]; pagination?: PgPagination }> {
    ...
    try {
      ...
      const [profiles, totalCount] =
        await this.riskProfileRepository.findAllProfiles(query, pagination);
  ...
  async createRiskProfiles(profilesData: CreateRiskProfileDto[]) {
    ...
    try {
      const profiles =
        await this.riskProfileRepository.repository.manager.transaction(
          async (manager) => {
      ...
      const result =
        await this.riskProfileRepository.repository.manager.transaction(
          async (manager) => {
            ...
            // Update risk profile
            const updatedProfile = manager.merge(
              RiskProfile,
              profile,
              profileInfo,
            );
            ...
            if (translations) {
              ...
              await manager.delete(RiskProfileTranslation, {
                riskProfileId: id,
              });
  ...
  private async clearProfileCache(): Promise<void> {
    try {
      ...
      this.logger.error(
        `[clearProfileCache] Error clearing profile cache: ${error.message}`,
      );
      throw error;
    }
  }
