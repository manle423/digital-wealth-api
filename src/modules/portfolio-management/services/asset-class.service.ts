...
export class AssetClassService {
  ...
  async getAllAssetClasses(
    query?: GetAssetClassesDto,
  ): Promise<{ data: AssetClass[]; pagination?: PgPagination }> {
    ...
    const [assetClasses, totalCount] =
      await this.assetClassRepository.findAllAssetClasses(query, pagination);
    ...
    const assetClassesWithTranslations = assetClasses.map((assetClass) => {
      const assetClassTranslations = translations.filter(
        (t) => t.assetClassId === assetClass.id,
      );
      return {
        ...assetClass,
        translations: assetClassTranslations,
        name: assetClassTranslations.find((t) => t.language === Language.VI)
          ?.name,
        description: assetClassTranslations.find(
  ...
  async getAssetClassById(id: string) {
    ...
    const assetClassWithTranslations = {
      ...
      description: translations.find((t) => t.language === Language.VI)
        ?.description,
    };
    ...
    await this.redisService.set(
      cacheKey,
      JSON.stringify(assetClassWithTranslations),
      RedisKeyTtl.THIRTY_DAYS,
    );
  ...
  async createAssetClasses(assetClassesData: CreateAssetClassDto[]) {
    this.logger.info('[createAssetClasses]', {
  ...
  async deleteAssetClass(id: string): Promise<boolean> {
    ...
    try {
      ...
      const result =
        await this.assetClassRepository.repository.manager.transaction(
          async (manager) => {
            // Xóa translations trước
            await manager.delete('risk_assessment_asset_class_translations', {
              assetClassId: id,
            });
  ...
  private async invalidateAssetClassCache(): Promise<void> {
    try {
      ...
      this.logger.debug(
        '[invalidateAssetClassCache] Asset class cache invalidated',
      );
    } catch (error) {
      this.logger.error(
        `[invalidateAssetClassCache] Error invalidating cache: ${error.message}`,
      );
    }
  }
}
