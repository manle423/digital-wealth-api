...
import {
  BatchUpdateAllocationDto,
  UpdateAssetAllocationDto,
} from '../dto/asset-allocation/update-asset-allocation.dto';
...
export class AssetAllocationService {
  ...
  async getAllocationsByRiskProfile(riskProfileId: string) {
    ...
    try {
      ...
      this.logger.error(
        `Error in getAllocationsByRiskProfile: ${error.message}`,
      );
      handleDatabaseError(
        error,
        'AssetAllocationService.getAllocationsByRiskProfile',
      );
    }
  }
  ...
  async createAllocation(createDtos: CreateAssetAllocationDto[]) {
    ...
    const uniqueRiskProfileIds = [
      ...new Set(createDtos.map((dto) => dto.riskProfileId)),
    ];
    await Promise.all(
      uniqueRiskProfileIds.map((id) =>
        this.riskProfileService.getRiskProfileById(id),
      ),
    );

    // Kiểm tra assetClass tồn tại cho tất cả các allocation
    const uniqueAssetClassIds = [
      ...new Set(createDtos.map((dto) => dto.assetClassId)),
    ];
    const assetClasses = await Promise.all(
      uniqueAssetClassIds.map((id) =>
        this.assetClassService.getAssetClassById(id),
      ),
    );
  ...
  async batchUpdateAllocations(dto: BatchUpdateAllocationDto) {
    ...
    try {
      ...
      const result =
        await this.assetAllocationRepository.save(allocationsToCreate);
      ...
      handleDatabaseError(
        error,
        'AssetAllocationService.batchUpdateAllocations',
      );
    }
  }
  ...
  private async invalidateAssetAllocationCache(): Promise<void> {
    try {
      ...
      this.logger.debug(
        '[invalidateAssetAllocationCache] Asset allocation cache invalidated',
      );
    } catch (error) {
      this.logger.error(
        `[invalidateAssetAllocationCache] Error invalidating cache: ${error.message}`,
      );
    }
  }
}
