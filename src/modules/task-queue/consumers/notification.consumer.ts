...
import { IWelcomeEmailData, IOtpEmailData } from '../interfaces/notification.interface';
...
export class NotificationConsumer {
  ...
  async handleOtpEmail(rawMessage: string) {
    try {
      ...
      // Validate message structure
      if (!message || !message.data || !message.data.otp || !message.data.email) {
        this.logger.error('Invalid message structure', { message });
        throw new Error('Invalid message structure');
      }

      // Render email template
      const emailContent = renderTemplate(message.template, {
      ...
      this.logger.error('Failed to process OTP email message', { error, rawMessage });
      throw error;
    }
  }
}