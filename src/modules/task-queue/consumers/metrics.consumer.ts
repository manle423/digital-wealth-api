...
export class MetricsConsumer {
  ...
  async handleCalculateMetrics(rawMessage: string) {
    try {
      this.logger.info('[handleCalculateMetrics] Processing metrics calculation message', { rawMessage });
      ...
      try {
        ...
        this.logger.error('[handleCalculateMetrics] Failed to parse message', { error: parseError, rawMessage });
        throw new Error('Invalid message format');
      }

      // Validate message structure
      if (!message || !message.userId) {
        this.logger.error('[handleCalculateMetrics] Invalid message structure', { message });
      ...
      this.logger.error('[handleCalculateMetrics] Failed to process metrics calculation', { error, rawMessage });
      throw error;
    }
  }
} 