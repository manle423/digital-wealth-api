...
export class RiskAssessmentService {
  ...
  async saveAssessmentResult(
    ...
    if (riskProfile) {
      const allocations =
        await this.assetAllocationRepository.findByRiskProfileId(
          riskProfile.id,
        );
      if (allocations && allocations.length > 0) {
        recommendedAllocation = allocations.map((allocation) => ({
          assetClass: allocation.assetClass.translations.find(
            (t) => t.language === Language.VI,
          )?.name,
    ...
    if (recommendedAllocation.length === 0) {
      recommendedAllocation =
        this.generateRecommendedAllocation(riskProfileType);
    }
    ...
    if (riskProfile?.translations?.length > 0) {
      ...
      const viTranslation = riskProfile.translations.find(
        (t) => t.language === Language.VI,
      );
      if (viTranslation) {
    ...
    const savedResults =
      await this.assessmentResultRepository.save(assessmentResultData);
  ...
  generateSummary(
    ...
    const profiles = {
      [RiskProfileType.CONSERVATIVE]:
        'Bạn có khẩu vị rủi ro thấp, ưu tiên bảo toàn vốn hơn là lợi nhuận cao. Chiến lược đầu tư của bạn nên tập trung vào các tài sản an toàn.',
      [RiskProfileType.MODERATELY_CONSERVATIVE]:
        'Bạn có khẩu vị rủi ro thấp đến trung bình, chấp nhận một chút rủi ro để có được lợi nhuận tốt hơn.',
      [RiskProfileType.MODERATE]:
        'Bạn có khẩu vị rủi ro trung bình, cân bằng giữa bảo toàn vốn và tăng trưởng.',
      [RiskProfileType.MODERATELY_AGGRESSIVE]:
        'Bạn có khẩu vị rủi ro trung bình đến cao, sẵn sàng chấp nhận rủi ro lớn hơn để đạt được lợi nhuận cao hơn.',
      [RiskProfileType.AGGRESSIVE]:
        'Bạn có khẩu vị rủi ro cao, ưu tiên tăng trưởng dài hạn và sẵn sàng chấp nhận biến động lớn trong ngắn hạn.',
    };
