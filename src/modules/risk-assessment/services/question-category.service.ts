...
export class QuestionCategoryService {
  ...
  async getCategories(query?: GetQuestionCategoriesDto) {
    ...
    try {
      ...
      const [categories, totalCount] =
        await this.questionCategoryRepository.findAllCategories(
          query,
          pagination,
        );
  ...
  async getCategoryById(id: string) {
    ...
    try {
      ...
      await this.redisService.set(
        cacheKey,
        JSON.stringify(categoryWithQuestions),
        RedisKeyTtl.THIRTY_DAYS,
      );
  ...
  async findAll(
    query?: GetQuestionCategoriesDto,
    pagination?: IPagination,
  ): Promise<[QuestionCategory[], number]> {
    this.logger.info('[findAll]', { query, pagination });
    try {
      return await this.questionCategoryRepository.findAllCategories(
        query,
        pagination,
      );
    } catch (error) {
  ...
  async createMultiple(
    createDto: CreateMultipleQuestionCategoriesDto,
  ): Promise<QuestionCategory[]> {
    this.logger.info('[createMultiple]', { categories: createDto.categories });
    try {
      if (
        !createDto.categories ||
        !Array.isArray(createDto.categories) ||
        createDto.categories.length === 0
      ) {
        throw new Error('No valid categories provided');
      }

      this.logger.info(
        `Creating multiple question categories: ${createDto.categories.length} items`,
      );
      this.logger.debug(
        `Categories data: ${JSON.stringify(createDto.categories)}`,
      );

      const result = await this.questionCategoryRepository.repository.save(
        createDto.categories,
      );
  ...
  async updateMultiple(
    updateDto: UpdateMultipleQuestionCategoriesDto,
  ): Promise<QuestionCategory[]> {
    this.logger.info('[updateMultiple]', { categories: updateDto.categories });
    try {
      if (
        !updateDto.categories ||
        !Array.isArray(updateDto.categories) ||
        updateDto.categories.length === 0
      ) {
        throw new Error('No valid categories to update');
      }

      this.logger.info(
        `Updating multiple question categories: ${updateDto.categories.length} items`,
      );
      const result =
        await this.questionCategoryRepository.updateMultipleCategories(
          updateDto.categories,
        );
  ...
  async removeMultiple(
    deleteDto: DeleteQuestionCategoriesDto,
  ): Promise<boolean> {
    this.logger.info('[removeMultiple]', { ids: deleteDto.ids });
    try {
      if (
        !deleteDto.ids ||
        !Array.isArray(deleteDto.ids) ||
        deleteDto.ids.length === 0
      ) {
        throw new Error('No valid IDs to remove');
      }

      this.logger.info(
        `Removing multiple question categories: ${deleteDto.ids.length} items`,
      );
      const result =
        await this.questionCategoryRepository.deleteMultipleCategories(
          deleteDto.ids,
        );
  ...
  private async clearCategoryCache(): Promise<void> {
    try {
      ...
      this.logger.error(
        `[clearCategoryCache] Error clearing category cache: ${error.message}`,
      );
      throw error;
    }
  }
